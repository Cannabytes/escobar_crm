...
- Любой авторизованный пользователь может:
  ...
  - начинать приватный диалог кликом по пользователю в списке контактов;
...
- Лента сообщений с автообновлением (каждые 2 секунды) и ручным обновлением.
...
- `GET /admin/chat/users/search?q=` — поиск пользователя для приватного чата (опционально, доступен для будущих UI-доработок).
