...
> Обновления сидеров (например, добавление новых разрешений) безопасны: данные синхронизируются через `updateOrCreate`, дублей не появится.
...
Раздел **Система → Роли и права** доступен всем пользователям, у которых есть разрешение `roles.view` или `roles.manage`.
...
3. Выберите разрешения:
   ...
   - Используйте быстрые действия: поиск, массовый выбор по типу, включение/отключение прав внутри группы
...
### Телефонный справочник
- `user-phones.view` — Просмотр телефонного справочника
- `user-phones.create` — Создание записи телефонного справочника
- `user-phones.edit` — Редактирование записи телефонного справочника
- `user-phones.delete` — Удаление записи телефонного справочника
- `user-phones.manage` — Полное управление телефонным справочником
...
1. **Управлять ролями** (создавать/редактировать/клонировать) могут пользователи с разрешением `roles.manage`
